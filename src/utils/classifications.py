...
class Classfication(object):
    @classmethod
    def init_parameters(cls, mode=CLASSFICATION_MODE.train, **kwargs):
        if mode == CLASSFICATION_MODE.train:
            tf.app.flags.DEFINE_string(
                'ckpt_save_dir', 'ckpt/inception_classification',
                """Directory for saving model checkpoint. """)

            tf.app.flags.DEFINE_string(
                'ckpt_restore_dir', 'ckpt/inception_classification',
                """Directory for restoring old model checkpoint. """)

            tf.app.flags.DEFINE_string(
                'pretrained_model_ckpt_path', 'ckpt/inception-v3/model.ckpt-157585',
        ...
        elif mode == CLASSFICATION_MODE.test:
            pass
        else:
            raise NotImplementedError('Unknown classification mode.')

    @classmethod
    def load_image(cls, path, mode=CLASSFICATION_MODE.train):
    ...
    @classmethod
    def train():
        cls.init_parameters(CLASSFICATION_MODE.train)
